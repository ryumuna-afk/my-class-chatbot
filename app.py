...
<style>
    ...
    .memo-board {
        ...
        margin-bottom: 20px;
        font-size: 16px;
        max-height: 150px; /* 🌟 최대 높이 지정 (약 5~6줄 크기) */
        overflow-y: auto;  /* 🌟 내용이 넘치면 스크롤바 생성 */
    }
    
    /* 예쁜 스크롤바 디자인 */
    .memo-board::-webkit-scrollbar {
        width: 6px;
    }
    .memo-board::-webkit-scrollbar-thumb {
...
# 📌 3.5 화이트보드 내용 불러오기 (UI 전시용 & AI 두뇌 연동용)
try:
...
with st.sidebar:
    ...
    if st.text_input("비밀번호 입력", type="password") == "0486":
        ...
        new_memo = st.text_area("학생들에게 띄울 알림장 내용을 적어주세요. (지우면 사라집니다)", value=board_text, height=150)
...
# 🌟 1. 학급 공식 채널 (드라이브, 클래스룸, 캘린더 3단 콤보!) - 실제 링크 적용됨!
...
if user_question := st.chat_input("👉 이곳을 터치해서 챗봇에게 대답이나 질문을 입력하세요! 👈"):
    ...
    # 📌 AI의 두뇌(프롬프트)에 화이트보드 내용 주입!
    if board_text.strip():
        system_prompt += f"\n[현재 담임 선생님의 화이트보드(알림장) 공지사항]\n{board_text}\n(※ AI 지침: 학생의 질문이 위 공지사항과 관련이 있다면, 공지 내용을 적극적으로 참고하여 대답하십시오.)\n"
        
    system_prompt += f"""
    [행동 수칙]
    1. 🚨단어/단답형 입력 시 되묻기 (가장 중요)🚨: 학생이 "수행평가", "동아리" 처럼 단어나 너무 짧은 문장만 입력하면 절대 혼자 추측해서 길게 대답하지 마십시오. 반드시 "수행평가 일정이 궁금해? 아니면 과제 제출 방법이 궁금해? 구체적으로 말해줄래?" 라고 되물어보십시오.
    2. 🚨모를 때의 대처 (엄격한 지침)🚨: 제공된 학교 자료나 화이트보드 공지에 없는 내용이라면, 절대 지어내거나(Hallucination) 일반론적인 대답을 하지 마십시오. 반드시 "제가 가진 자료에는 그 내용이 없네요 ㅠㅠ 선생님께 직접 여쭤보는 건 어떨까?" 라고 대답하십시오.
    3. 🚨주제 이탈 금지🚨: 만약 학생의 질문이 학교생활, 진로 탐색, 상급학년 준비, 독서와 관련 없는 내용이라면 단호하게 끊어주십시오.
    4. 🌟꼬.꼬.독 모드 멘토링 지침🌟: 
       - 현재 주제가 '④ 📚 꼬.꼬.독 (진로 독서)'일 경우, 당신은 유능한 독서 멘토입니다.
       - (1단계-공감) 학생의 마지막 대답을 인용하며 공감/칭찬합니다. 
       - (2단계-조언) 짧고 유익한 새로운 시각을 제공합니다.
       - (3단계-꼬리질문) 조언을 바탕으로 한 단계 더 깊이 파고드는 '꼬리 질문 딱 1개'를 던지십시오.
    5. 🌟홀랜드 맞춤형 컨설팅🌟: 학생이 진로나 선택과목을 물어볼 때, 학생의 홀랜드 유형({student_holland})을 반영하여 추천해 주십시오. (정보가 없으면 생략)
    6. 🌟드라이브 폴더 안내🌟: 실시간 학교 자료 목록에 관련된 파일명이 있다면, "💡 자세한 내용은 구글 드라이브의 **[관련 파일명]**을 확인해 봐~" 라고 안내하십시오.
