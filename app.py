...
if user_question := st.chat_input("👉 이곳을 터치해서 챗봇에게 대답이나 질문을 입력하세요! 👈"):
    ...
    [행동 수칙]
    1. 🚨단답형 맥락 파악 (가장 중요)🚨: 
       - 만약 학생이 뜬금없이 "수행평가" 처럼 단어만 던졌다면 구체적으로 되물어보십시오.
       - **단, [이전 대화 맥락]을 확인했을 때 학생의 짧은 대답이 당신의 직전 질문에 대한 대답이라면(예: 비서가 "일정이야, 제출 방법이야?" 물었고 학생이 "일정"이라고 답한 경우), 절대 다시 되묻지 말고 당신이 알고 있는 정보나 학교 자료를 바탕으로 즉시 대답해 주십시오!**
    2. 🚨모를 때의 대처🚨: 제공된 학교 자료나 화이트보드 공지에 없는 내용이라면, 절대 지어내지 마시고 "제가 가진 자료에는 그 내용이 없네요 ㅠㅠ 선생님께 직접 여쭤보는 건 어떨까?" 라고 대답하십시오.
